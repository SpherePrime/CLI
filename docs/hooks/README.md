...
the agent lifecycle, allowing you to both build on top of Prime, customize
...
- Prime ships with a builtin `prime-hook` skill write, edit, and configure
  hooks; just tell Prime how to configure Prime
- Prime currently supports just one hook, `PreToolUse`, with plans to support
...
our **project-level** `options.json`. Relative paths like `./no-haskell.sh` work
here because the project root is your working directory. If you're configuring
a global hook (`~/.config/prime/options.json`), use an absolute path instead.
...
# ($PRIME_TOOL_INPUT_COMMAND) to grep and match on a regexp.
if echo "$PRIME_TOOL_INPUT_COMMAND" | grep -qE '(^| )((ghc|cabal|stack)(\.exe)?)( |$)'; then
...
Hooks run through Prime's embedded POSIX shell (`mvdan.cc/sh`) — the same
...
- **Permissive shebang fallback**: if the absolute path in a shebang
  doesn't exist (e.g. `#!/bin/bash` on Windows), Prime falls back to a
  ...
  "no opinion".
- **Environment**: every hook sees `PRIME=1`, `AGENT=prime`, and
  `AI_AGENT=prime` on top of the `PRIME_*` hook-specific variables. These
  ...
  both contexts.
- **Timeout behavior**: when a hook exceeds its timeout, Prime cancels the
  context and waits a short grace period (~1s) for the interpreter to
  yield. If the hook still hasn't returned, Prime abandons it, logs a
...
Hooks live in the `hooks` section of `options.json` (or a `primerc`) at both the
...
> work fine in project-level `options.json` because the project root is also
> your working directory. For **global** config (`~/.config/prime/`),
> however, you must use either an absolute path or an inline command:
>
> ```jsonc
> // Global ~/.config/prime/options.json
> {
>   "hooks": {
>     "PreToolUse": [
>       {
>         "command": "/home/you/.config/prime/hooks/no-haskell.sh"
...
3. Runs all matching hooks **in parallel** through Prime's embedded POSIX
...
5. Applies the result **before** permission checks. If the aggregated decision
   ...
   for it. If it's `allow`, Prime treats that as affirmative pre-approval and
...
however you'll incur some additional overhead as Prime will still parse and
run each hook.
...
| ---------------------------- | ---------------------------------------------- |
| `PRIME`                      | Always `1` when running under Prime.           |
| `AGENT`                      | Always `prime`.                                |
| `AI_AGENT`                   | Always `prime`.                                |
| `PRIME_EVENT`                | The hook event name (e.g. `PreToolUse`).       |
| `PRIME_TOOL_NAME`            | The tool being called (e.g. `bash`).           |
| `PRIME_SESSION_ID`           | Current session ID.                            |
| `PRIME_CWD`                  | Working directory.                             |
| `PRIME_PROJECT_DIR`          | Project root directory.                        |
| `PRIME_TOOL_INPUT_COMMAND`   | For `bash` calls: the shell command being run. |
| `PRIME_TOOL_INPUT_FILE_PATH` | For file tools: the target file path.          |

The `PRIME`, `AGENT`, and `AI_AGENT` markers are also set by the `bash`
tool, so a script can detect "am I running under Prime?" the same way in
either context.
...
Hooks communicate back to Prime via **exit code** and `stdout`/`stderr`. The
...
deterministic based on the order hooks appear in `options.json`.
...
If a hook exceeds its timeout, Prime cancels its context and treats the
...
if echo "$PRIME_TOOL_INPUT_COMMAND" | grep -qE 'rm\s+-(rf|fr)\s+/'; then
...
returns `decision: "allow"`, which tells Prime to pre-approve the call:
...
case "$PRIME_TOOL_INPUT_COMMAND" in
...
if [[ "$PRIME_TOOL_INPUT_FILE_PATH" == *.go ]]; then
...
```jsonc
{ "command": "echo \"$(date -Iseconds) $PRIME_TOOL_NAME\" >> ./tools.log" }
```
...
Prime hooks are broadly compatible with [Claude Code
...
hooks run under Prime unchanged. This document covers the Prime-specific API
only — anything not documented here isn't guaranteed to work.

One intentional divergence: Prime treats `updated_input` as a shallow-merge
...
4. `decision` precedence: `deny` > `allow` > `null`. First deny determines the
   ...
   is `allow`, Prime pre-approves the tool call and skips the permission
