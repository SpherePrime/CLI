...
Right now `primerc` runs once, at startup. If you want to change something
...
- To make it stick, you edit your `primerc` — like editing `.bashrc`.

So you could say "Prime, switch to the small model for a bit" and it just
...
> `primerc` from live config state. Want it permanent? Edit `primerc`.
...
Prime currently uses JSON files in data directories as both persisted machine
state and high-priority configuration:

```text
~/.local/share/prime/prime.json
.prime/prime.json
```
...
`primerc` and legacy `prime.json` files.

The goal is to make the roles explicit:

| Role | Format |
|---|---|
| User-authored executable configuration | `primerc` / `.primerc` |
| Legacy user-authored static configuration | `prime.json` / `.prime.json` |
| Prime-owned persistent preferences and history | versioned `state.json` |
...
```text
~/.config/prime/primerc             global user config
~/.local/share/prime/state.json     global machine state
./primerc / ./.primerc              project user config
.prime/state.json                   workspace machine state
```
...
→ global legacy prime.json
→ global primerc
→ project legacy prime.json
→ project primerc
→ project .prime.json
→ project .primerc
...
3. Provide an explicit conversion command (for example,
   `prime config convert prime.json > primerc`).
...
### Typed primerc builder

This is related but separate. Today the Bash path is:

```text
primerc → map[string]any → JSON → Config
```

A later typed-builder phase should become:

```text
primerc → typed ConfigBuilder → Config
state.json → typed StateStore ───────┘
```

Legacy `prime.json` would decode into a typed config patch and apply to the same
...
7. Replace the `primerc` map/JSON bridge with a typed config builder.
...
`prime.json.migrated`; corrupt files should be quarantined as timestamped
...
Prime currently has three useful tool states across both config formats:

| State | `primerc` | `prime.json` | Behavior |
|---|---|---|---|
...
The one state Prime does **not** have is "visible but always rejected": the
