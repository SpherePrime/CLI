...
> [!TIP]
>
> Prime can configure itself via a builtin config skill. That is to say,
> can generally just tell Prime want you want to configure using natural
> language.
>
> If you're migrating from the old JSON format, you can also ask Prime to
> convert the config for you.

Prime is configured with Bash via a set of Prime-specific builtin commands. By
default, global config lives at `~/.config/prime/primerc` on Unix-like systems
and `%USERPROFILE%\.config\prime\primerc` on Windows. It works like a `.bashrc`:
it runs when Prime starts and configures the agent.
...
1. Prime ships with a first-class Bash interpreter, so we get the logic for
   free.
2. Ultimately, Prime needs to be able to configure itself, and command-based
...
target specific Prime versions with `$PRIME_VERSION`:

```bash
if [[ $PRIME_VERSION == "0.85.*" ]]; then
    option debug true
fi
```
...
> (`%LOCALAPPDATA%\prime` on Windows). This is application state, and should
...
Available Commands:
  ...
  option        Configure general Prime behavior
```
...
same `<provider>/<id>` form printed by `prime models`.
...
Configure general Prime behavior, paths, attribution, and the terminal UI.
...
Boolean Keys:
  ...
  attribution-generated-with     add the Generated with Prime line

String Keys:
  data-directory string            directory for project data and state
  initialize-as string             context filename created by prime init
...
Available Keys:
  ...
  exit-banner default|compact|none
                                ...
                                the Prime logo, compact shows only the resume
...
```bash
# Unix-like: ~/.config/prime/primerc
# Windows:   %USERPROFILE%\.config\prime\primerc
source ~/team/prime-base.sh    # sets up providers, a few skills
...
headers) are shell-expanded at load time. In `primerc` there's no such list —
it's all just Bash.
