...
name: builtin-skills
description:
  Use when creating a new builtin skill for Prime, editing an existing builtin
...
Prime embeds skills directly into the binary via `internal/skills/builtin/`.
...
- `DiscoverBuiltin()` walks the embedded FS, parses each `SKILL.md`, and sets
  paths with the `prime://skills/` prefix (e.g., `prime://skills/jq/SKILL.md`).
- The View tool resolves `prime://` paths from the embedded FS, not disk.
...
| -------------- | ----------------------- | ------------------------------------------ |
| `prime-config` | `builtin/prime-config/` | Prime configuration help                   |
| `prime-hooks`  | `builtin/prime-hooks/`  | Authoring, configuring and debugging hooks |
