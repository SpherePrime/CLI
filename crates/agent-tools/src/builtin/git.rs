...
pub fn git_tool() -> crate::ToolDefinition {
    crate::ToolDefinition {
        name: "git".into(),
        description: "Inspect or modify git state. Actions: status, diff, log, branches, add, commit, checkout, push, pull, fetch, stash, show, blame, reset, remote, run.".into(),
        input_schema: json!({
            "type": "object",
            "properties": {
                "action": {
                    "type": "string",
                    "enum": ["status", "diff", "log", "branches", "add", "commit", "checkout", "branch", "push", "pull", "fetch", "stash", "show", "blame", "reset", "remote", "run"]
                },
                "staged": { "type": "boolean" },
                "limit": { "type": "integer" },
