...
pub fn shorten_path(path: &str, max_len: usize) -> String {
    let path_str =
        if path.starts_with("C:\\") || path.starts_with("D:\\") || path.starts_with("E:\\") {
            let rest = &path[2..];
            rest.strip_prefix('\\').unwrap_or(path)
...
pub fn render_header(
    ...
    let status_line = format!("✓ Ready  model  {model}  provider  {provider}  📁 {cwd_short}");
...
pub fn render_status_line(_frame: &mut Frame, _area: Rect, _status_text: &str, _theme: &Theme) {}
