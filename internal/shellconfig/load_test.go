...
func TestLoadShellConfig_Provider(t *testing.T) {
	...
	path := filepath.Join(dir, "primerc")
...
func TestLoadShellConfig_Option(t *testing.T) {
	...
	dir := t.TempDir()
	script := `option data-directory .prime
...
option debug`
	...
	opts := result["options"].(map[string]any)
	require.Equal(t, ".prime", opts["data_directory"])
...
// TestLoadShellConfig_PrimeVersionEnv verifies that PRIME_VERSION is exposed
// to the script so it can feature-detect the running Prime version.
func TestLoadShellConfig_PrimeVersionEnv(t *testing.T) {
	dir := t.TempDir()
	script := `provider add openai --api-key "$PRIME_VERSION"`
	path := filepath.Join(dir, "primerc")
...
func TestLoadShellConfig_ProviderJSONFlagsRequireObjects(t *testing.T) {
	...
	path := filepath.Join(t.TempDir(), "primerc")
...
// primerc cannot block config loading indefinitely. Config loads run on the
...
func TestLoadShellConfig_RespectsContextCancellation(t *testing.T) {
	...
	select {
	case err := <-done:
		require.Error(t, err, "a cancelled primerc must fail, not succeed")
