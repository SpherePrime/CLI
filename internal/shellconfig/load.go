...
// LoadShellConfig executes a primerc script and returns its config as a
...
func LoadShellConfig(ctx context.Context, path string, src []byte) ([]byte, error) {
	...
	// Expose the running Prime version so scripts can feature-detect, e.g.
	// [[ "$PRIME_VERSION" == "devel" ]] or branch on the release.
	env := append(os.Environ(), "PRIME_VERSION="+version.Version)
