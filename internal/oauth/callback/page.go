...
//go:embed page.html page.css page.js heartbit.svg heartbit-grumpy.svg logo.svg
var assets embed.FS
...
func Write(w io.Writer, r Result) error {
	...
	logo, err := assets.ReadFile("logo.svg")
	if err != nil {
	...
	if r.ContinueURL != "" {
		...
		data.Title = "Authorization failed — Prime"
		data.Kind = "failed"
		data.Heading = "Authorization failed"
		data.Detail = "Prime was not granted access to"
		if r.Subject == "" {
			data.Detail = "Prime was not granted access."
		}
		// A failed page keeps itself open: the reader needs the reason,
		// and closing the tab out from under them would take it away.
		data.Status = "Close this tab and try again from Prime."
	} else {
		data.Title = "Authorized — Prime"
		data.Kind = "ok"
		data.Heading = "You’re all set"
		data.Detail = "Prime is now connected to"
		if r.Subject == "" {
			data.Detail = "Prime is now connected."
		}
