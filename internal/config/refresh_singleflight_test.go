...
func isolateHyperCredentials(t *testing.T) {
	...
	t.Setenv("PRIME_HYPER_API_KEY", "")
}

// writeTokenToDisk persists token as the hyper provider credential in the
// config file at path, mimicking what another prime instance would leave
...
// which lets a single test process faithfully simulate two prime instances:
...
func TestRefreshOAuthToken_InProcessSingleFlight(t *testing.T) {
	...
	configPath := filepath.Join(t.TempDir(), "prime.json")
