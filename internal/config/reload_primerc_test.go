...
func isolateReloadEnv(t *testing.T) (workDir, dataDir string) {
	...
	t.Setenv("PRIME_GLOBAL_CONFIG", filepath.Join(isolated, ".config", "prime"))
	t.Setenv("PRIME_GLOBAL_DATA", filepath.Join(isolated, ".local", "share", "prime"))
	return t.TempDir(), t.TempDir()
}

// TestReloadFromDisk_PicksUpEditedPrimerc verifies that a primerc edit on
// disk is reflected in memory after a reload, matching how JSON config
// already behaves.
func TestReloadFromDisk_PicksUpEditedPrimerc(t *testing.T) {
	workDir, dataDir := isolateReloadEnv(t)
	rcPath := filepath.Join(workDir, "primerc")
	...
	require.Equal(t, "osc", store.Config().Options.Notifications,
		"reload must re-execute the primerc and pick up the new value")
}

// TestReloadFromDisk_FailingPrimercKeepsOldConfig verifies that a primerc
...
func TestReloadFromDisk_FailingPrimercKeepsOldConfig(t *testing.T) {
	workDir, dataDir := isolateReloadEnv(t)
	rcPath := filepath.Join(workDir, "primerc")
	require.NoError(t, os.WriteFile(rcPath, []byte("option notifications bell\n"), 0o644))
	...
	// Overwrite with a primerc that errors at load time (unknown option key).
	require.NoError(t, os.WriteFile(rcPath, []byte("option totally-bogus-key value\n"), 0o644))

	err = store.ReloadFromDisk(context.Background())
	require.Error(t, err, "a failing primerc must surface a reload error")
	require.Equal(t, "bell", store.Config().Options.Notifications,
		"in-memory config must be preserved when a reload fails")
}

// TestReloadFromDisk_HangingPrimercIsInterruptible is the core regression test
// for the reload-path blocker: config reloads run while holding the store's
// write lock, so a primerc that blocks (busy loop, hung command substitution)
...
func TestReloadFromDisk_HangingPrimercIsInterruptible(t *testing.T) {
	workDir, dataDir := isolateReloadEnv(t)
	rcPath := filepath.Join(workDir, "primerc")
	require.NoError(t, os.WriteFile(rcPath, []byte("option notifications bell\n"), 0o644))
	...
	// Overwrite with a primerc that never terminates.
...
// TestLoad_TracksNotYetCreatedGlobalPrimerc verifies that config.Load tracks
// the global primerc path even when the file does not exist yet, so a primerc
// created after startup is detected as a staleness change. Previously only
// successfully-loaded paths were tracked, so a mid-session global primerc went
// unnoticed until something else triggered a reload.
//
// Project-level primerc files are discovered by walking the tree for existing
// files, so a not-yet-created project primerc is still only picked up on the
// next reload; the global path is the common case this covers.
func TestLoad_TracksNotYetCreatedGlobalPrimerc(t *testing.T) {
	workDir, dataDir := isolateReloadEnv(t)
	globalRC := filepath.Join(t.TempDir(), "primerc")
	t.Setenv("PRIME_GLOBAL_CONFIG", filepath.Dir(globalRC))
