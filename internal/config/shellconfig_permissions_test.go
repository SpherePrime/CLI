...
// loadPrimeSh writes a prime.sh into an isolated project and loads it through
...
func loadPrimeSh(t *testing.T, script string) *config.ConfigStore {
	t.Helper()
	store, err := loadPrimeShErr(t, script)
	require.NoError(t, err)
	return store
}

// loadPrimeShErr is loadPrimeSh without asserting success, for cases that are
// expected to fail at load time.
func loadPrimeShErr(t *testing.T, script string) (*config.ConfigStore, error) {
	t.Helper()
	...
	workDir := t.TempDir()
	dataDir := t.TempDir()
	require.NoError(t, os.WriteFile(filepath.Join(workDir, "primerc"), []byte(script), 0o644))

	return config.Load(workDir, dataDir, false)
}

func TestShellConfigPermissionsAllow(t *testing.T) {
	store := loadPrimeSh(t, `permissions allow bash view`)
...
func TestShellConfigPermissionsAccumulateAndDedup(t *testing.T) {
	store := loadPrimeSh(t, `permissions allow bash
...
func TestShellConfigPermissionsDeny(t *testing.T) {
	store := loadPrimeSh(t, `permissions deny bash sourcegraph
