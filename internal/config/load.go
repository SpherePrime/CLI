...
import (
	...
	"github.com/SpherePrime/CLI/internal/shellconfig"
	powernapConfig "github.com/SpherePrime/CLI/vendordeps/dwertyfa288/x/powernap/pkg/config"
...
func Load(workingDir, dataDir string, debug bool) (*ConfigStore, error) {
	...
	// (e.g. a primerc added mid-session) is detected as a change.
...
func PushPopPrimeEnv() func() {
	var found []string
	for _, ev := range os.Environ() {
		if strings.HasPrefix(ev, "PRIME_") {
			pair := strings.SplitN(ev, "=", 2)
			if len(pair) != 2 {
				continue
			}
			found = append(found, strings.TrimPrefix(pair[0], "PRIME_"))
		}
	}
	...
	for _, ev := range found {
		os.Setenv(ev, os.Getenv("PRIME_"+ev))
	}
...
// wire from a Prime server need the same treatment before the UI reads them.
//
...
func (c *Config) setDefaults(workingDir, dataDir string) {
	...
	if len(c.Options.GlobalContextPaths) == 0 {
		primeConfigDir := filepath.Dir(GlobalConfig())
		c.Options.GlobalContextPaths = []string{
			filepath.Join(primeConfigDir, "PRIME.md"),
			filepath.Join(filepath.Dir(primeConfigDir), "AGENTS.md"),
		}
	}
	...
	// an MCP from prime.json. See MCPConfig.isOrphanedToken.
	...
	if str, ok := os.LookupEnv("PRIME_DISABLE_PROVIDER_AUTO_UPDATE"); ok {
		c.Options.DisableProviderAutoUpdate, _ = strconv.ParseBool(str)
	}

	if str, ok := os.LookupEnv("PRIME_DISABLE_DEFAULT_PROVIDERS"); ok {
...
// so an unrelated prime.json placed above the project is never picked
...
func loadFromConfigPaths(ctx context.Context, configPaths []string) (*Config, []string, error) {
	...
	// Track directories that have both prime.json and primerc to warn
	...
	// Warn if both a JSON config and a primerc exist in the same directory
	...
	for dir, jKeys := range jsonDirKeys {
		...
		if len(conflicts) > 0 {
			...
			slog.Warn("Found both a JSON config and a primerc in the same directory; merging with primerc taking precedence",
...
func GlobalConfig() string {
	if primeGlobal := os.Getenv("PRIME_GLOBAL_CONFIG"); primeGlobal != "" {
		return filepath.Join(primeGlobal, fmt.Sprintf("%s.json", appName))
	}
	return filepath.Join(home.Config(), appName, fmt.Sprintf("%s.json", appName))
}

// shellConfigSibling returns the primerc path that sits alongside a given
// prime.json path (same directory). Used so global config locations pick up a
...
// isShellConfig reports whether a config path is a shell config (primerc or
// the hidden .primerc), as opposed to a JSON config.
...
func GlobalCacheDir() string {
	if primeCache := os.Getenv("PRIME_CACHE_DIR"); primeCache != "" {
		return primeCache
	}
...
func GlobalConfigData() string {
	if primeData := os.Getenv("PRIME_GLOBAL_DATA"); primeData != "" {
		return filepath.Join(primeData, fmt.Sprintf("%s.json", appName))
	}
	...
	// for windows, it should be in `%LOCALAPPDATA%/prime/`
	// for linux and macOS, it should be in `$HOME/.local/share/prime/`
	if runtime.GOOS == "windows" {
		localAppData := cmp.Or(
			os.Getenv("LOCALAPPDATA"),
			filepath.Join(os.Getenv("USERPROFILE"), "AppData", "Local"),
...
// fallback keeps Prime from silently adopting state files placed above
// the current project.
...
func GlobalSkillsDirs() []string {
	if primeSkills := os.Getenv("PRIME_SKILLS_DIR"); primeSkills != "" {
		return []string{primeSkills}
	}
	...
	// On Windows, also load from app data on top of `$HOME/.config/prime`.
...
// ProjectSkillsDir returns the default project directories for which Prime
