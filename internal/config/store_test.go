...
func TestConfigStore_ConfigPath_GlobalAlwaysWorks(t *testing.T) {
	...
	store := &ConfigStore{
		globalDataPath: "/some/global/prime.json",
	}

	path, err := store.configPath(ScopeGlobal)
	require.NoError(t, err)
	require.Equal(t, "/some/global/prime.json", path)
}
...
func TestConfigStore_ConfigPath_WorkspaceReturnsPath(t *testing.T) {
	...
	store := &ConfigStore{
		workspacePath: "/some/workspace/.prime/prime.json",
	}

	path, err := store.configPath(ScopeWorkspace)
	require.NoError(t, err)
	require.Equal(t, "/some/workspace/.prime/prime.json", path)
}
...
// TestSetConfigFields_ProviderDottedKeys verifies the dotted-path pattern
// used by the TUI "Add Provider" dialog: each provider field is written as
// its own "providers.<id>.<field>" key so sjson.Set cannot clobber sibling
// keys, and struct-slice values (model lists) survive the round-trip.
func TestSetConfigFields_ProviderDottedKeys(t *testing.T) {
	t.Parallel()

	// Keep the data directory out of the config file's directory: when
	// they are the same file, the workspace config merge would append
	// the model array to itself.
	dir := t.TempDir()
	configPath := filepath.Join(dir, "prime.json")
	require.NoError(t, os.WriteFile(configPath, []byte(`{}`), 0o600))

	store, err := Load(dir, filepath.Join(dir, "state"), false)
	require.NoError(t, err)
	store.globalDataPath = configPath
	store.CaptureStalenessSnapshot([]string{configPath})
	...
	id := "providers.my-llm"
	models := []any{
		map[string]any{"id": "model-a", "name": "Model A", "context_window": 270000},
		map[string]any{"id": "model-b", "name": "Model B", "context_window": 270000},
	}
	err = store.SetConfigFields(ScopeGlobal, map[string]any{
		id + ".name":            "my-llm",
		id + ".base_url":        "https://api.my-llm.com/v1",
		id + ".api_key":         "sk-test",
	...
	raw := parsed["providers"].(map[string]any)["my-llm"].(map[string]any)
	require.Equal(t, "my-llm", raw["name"])
	require.Equal(t, "https://api.my-llm.com/v1", raw["base_url"])
	require.Equal(t, "sk-test", raw["api_key"])
	require.Equal(t, "openai-compat", raw["type"])
	require.Equal(t, true, raw["discover_models"])
	diskModels := raw["models"].([]any)
	require.Len(t, diskModels, 2)
	require.Equal(t, "model-a", diskModels[0].(map[string]any)["id"])
	require.Equal(t, float64(270000), diskModels[0].(map[string]any)["context_window"])
	require.Equal(t, "model-b", diskModels[1].(map[string]any)["id"])

	// The in-memory provider must reflect the saved fields. Discovery
	// re-runs against the unreachable example URL during the auto-reload
	// and can drop or replace models, so only check identity fields here.
	provider, ok := store.config.Providers.Get("my-llm")
	require.True(t, ok, "provider must be present in memory")
	require.Equal(t, "my-llm", provider.Name)
	require.Equal(t, "https://api.my-llm.com/v1", provider.BaseURL)
