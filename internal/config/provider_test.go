...
func TestProviders_Integration_WithMockClients(t *testing.T) {
	...
	catwalkPath := tmpDir + "/prime/providers.json"
	hyperPath := tmpDir + "/prime/hyper.json"
...
func TestProviders_Integration_WithCachedData(t *testing.T) {
	...
	require.NoError(t, os.MkdirAll(tmpDir+"/prime", 0o755))
...
func TestCachePathFor(t *testing.T) {
	tests := []struct {
		name            string
		primeGlobalData string
		xdgDataHome     string
		expected        string
	}{
		{
			name:            "with PRIME_GLOBAL_DATA",
			primeGlobalData: "/scratch/data",
			expected:        "/scratch/data/providers.json",
		},
		{
			name:            "PRIME_GLOBAL_DATA takes priority over XDG_DATA_HOME",
			primeGlobalData: "/scratch/data",
		...
		},
		{
			...
			xdgDataHome: "/custom/data",
			expected:    "/custom/data/prime/providers.json",
		},
		{
			name:     "without either",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Setenv("PRIME_GLOBAL_DATA", tt.primeGlobalData)
			...
			if tt.expected != "" {
				...
				require.Contains(t, result, "prime")
...
// several Prime instances depend on: the provider cache is swapped into place
