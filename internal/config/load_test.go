...
func TestLookupConfigs_BoundedByProject(t *testing.T) {
	...
	globalDir := t.TempDir()
	dataDir := t.TempDir()
	t.Setenv("PRIME_GLOBAL_CONFIG", globalDir)
	t.Setenv("PRIME_GLOBAL_DATA", dataDir)

	t.Run("does not pick up prime.json above non-git project", func(t *testing.T) {
		parent := t.TempDir()

		// prime.json above the project must not be adopted.
		require.NoError(t, os.WriteFile(
			filepath.Join(parent, "prime.json"),
			[]byte(`{}`),
			0o644,
		))
		...
		for _, p := range got {
			require.NotEqual(t, filepath.Join(parent, "prime.json"), p)
		}
	})

	t.Run("does not climb out of git worktree to find prime.json", func(t *testing.T) {
		...
		strayEval, err := filepath.EvalSymlinks(filepath.Join(parent, "prime.json"))
		require.NoError(t, err)
		for _, p := range got {
			...
			require.NotEqual(t, strayEval, pEval, "must not adopt parent prime.json")
		}
	})

	t.Run("picks up prime.json inside the project", func(t *testing.T) {
		project := t.TempDir()
		local := filepath.Join(project, "prime.json")
		...
		require.True(t, foundLocal, "expected project prime.json to be in lookup result: %v", got)
	})
	...
	t.Run("global shell config (primerc) is included", func(t *testing.T) {
		project := t.TempDir()

		got := lookupConfigs(project)
		// A global primerc is discovered only beside the user config. The data
		// directory is machine-owned state and must never execute a primerc.
		require.Contains(t, got, shellConfigSibling(GlobalConfig()))
		require.NotContains(t, got, shellConfigSibling(GlobalConfigData()))
	})

	t.Run("project primerc and .primerc are discovered", func(t *testing.T) {
		project := t.TempDir()
		require.NoError(t, os.WriteFile(filepath.Join(project, "primerc"), []byte(""), 0o644))
		require.NoError(t, os.WriteFile(filepath.Join(project, ".primerc"), []byte(""), 0o644))

		got := lookupConfigs(project)
		require.Contains(t, got, filepath.Join(project, "primerc"))
		require.Contains(t, got, filepath.Join(project, ".primerc"))
	})
	...
	t.Run("system config is loaded first", func(t *testing.T) {
		...
		require.Equal(t, "/etc/prime/prime.json", got[0])
	})
}
...
// config and a primerc coexist in the same directory, the merge warning names
...
func TestLoadFromConfigPaths_ConflictWarningNamesKeys(t *testing.T) {
	...
	t.Run("names overlapping keys", func(t *testing.T) {
		buf := capture(t)
		tmpDir := t.TempDir()
		jsonPath := filepath.Join(tmpDir, "prime.json")
		rcPath := filepath.Join(tmpDir, "primerc")
		...
		require.NoError(t, err)
		require.Contains(t, buf.String(), "primerc taking precedence")
	...
	t.Run("no warning when nothing overlaps", func(t *testing.T) {
		...
		_, _, err := loadFromConfigPaths(context.Background(), []string{jsonPath, rcPath})
		require.NoError(t, err)
		require.NotContains(t, buf.String(), "primerc taking precedence",
...
func TestConfig_setDefaults(t *testing.T) {
	t.Run("sets default data directory", func(t *testing.T) {
		...
		require.Equal(t, filepath.Join(workingDir, ".prime"), cfg.Options.DataDirectory)
	...
	t.Run("does not adopt .prime from a parent project", func(t *testing.T) {
		parent := t.TempDir()

		// .prime in the parent: it should not be reused by the child
	...
	t.Run("does not climb out of git worktree to find .prime", func(t *testing.T) {
		if _, err := exec.LookPath("git"); err != nil {
			t.Skip("git not available")
		}
		...
		// Stray .prime above the worktree root.
		require.NoError(t, os.Mkdir(filepath.Join(parent, defaultDataDirectory), 0o755))
		...
		require.NoError(t, err)
		require.NotEqual(t, strayEval, gotEval, "must not adopt parent .prime")
...
func TestConfig_setupAgentsWithEveryReadOnlyToolDisabled(t *testing.T) {
	...
	coderAgent, ok := cfg.Agents[AgentCoder]
	require.True(t, ok)
	assert.Equal(t, []string{"bash", "prime_info", "prime_logs", "job_output", "job_kill", "download", "edit", "multiedit", "lsp_diagnostics", "lsp_references", "lsp_restart", "lsp_rename", "lsp_replace_symbol", "fetch", "agentic_fetch", "question", "todos", "write", "list_mcp_resources", "read_mcp_resource"}, coderAgent.AllowedTools)
...
func TestConfig_setDefaultsDisableDefaultProvidersEnvVar(t *testing.T) {
	t.Run("sets option from environment variable", func(t *testing.T) {
		t.Setenv("PRIME_DISABLE_DEFAULT_PROVIDERS", "true")
...
func TestConfig_configureSelectedModels(t *testing.T) {
	t.Run("reload mode should not persist fallback defaults", func(t *testing.T) {
		dir := t.TempDir()
		globalPath := filepath.Join(dir, "prime.json")
