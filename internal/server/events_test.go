...
// are what non-interactive clients (e.g. `prime run`) rely on to
...
// through the SSE envelope. Without these correlators, `prime run`
...
// "agent failed" (returns non-zero from `prime run`) from "agent
