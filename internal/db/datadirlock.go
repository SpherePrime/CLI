...
// already in use by another prime process.
var ErrDataDirLocked = errors.New("data directory already in use by another prime process")

// dataDirLockFile is the name of the lock file inside the data
// directory. It lives next to prime.db so users can `ls` and find it.
const dataDirLockFile = "prime.lock"
...
// {dataDir}/prime.lock. If the lock is already held by another
...
// PRIME_SKIP_DATADIR_LOCK is set to a truthy value. This is intended
...
func skipDataDirLock() bool {
	v, _ := strconv.ParseBool(os.Getenv("PRIME_SKIP_DATADIR_LOCK"))
	return v
}
