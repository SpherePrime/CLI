...
func TestBuildEnv(t *testing.T) {
	...
	require.Equal(t, EventPreToolUse, envMap["PRIME_EVENT"])
	require.Equal(t, "bash", envMap["PRIME_TOOL_NAME"])
	require.Equal(t, "sess-1", envMap["PRIME_SESSION_ID"])
	require.Equal(t, "/work", envMap["PRIME_CWD"])
	require.Equal(t, "/project", envMap["PRIME_PROJECT_DIR"])
	require.Equal(t, "ls", envMap["PRIME_TOOL_INPUT_COMMAND"])
	require.Equal(t, "/tmp/f.txt", envMap["PRIME_TOOL_INPUT_FILE_PATH"])

	// Shared Prime markers must be present so hook-authored scripts can
	// detect they're running under Prime the same way bash-tool-invoked
	// scripts can.
	require.Equal(t, "1", envMap["PRIME"])
	require.Equal(t, "prime", envMap["AGENT"])
	require.Equal(t, "prime", envMap["AI_AGENT"])
}
...
func TestRunnerEnvVarsPropagated(t *testing.T) {
	t.Parallel()
	hookCfg := config.HookConfig{
		Command: `printf '{"decision":"allow","context":"%s"}' "$PRIME_TOOL_NAME"`,
	}
	r := NewRunner([]config.HookConfig{hookCfg}, t.TempDir(), t.TempDir())
	result, err := r.Run(context.Background(), EventPreToolUse, "sess", "bash", `{}`)
	require.NoError(t, err)
...
func TestParseStdoutClaudeCodeFormat(t *testing.T) {
	...
	t.Run("prime format still works", func(t *testing.T) {
		t.Parallel()
