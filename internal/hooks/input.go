...
func BuildEnv(eventName, toolName, sessionID, cwd, projectDir, toolInputJSON string) []string {
	env := os.Environ()
	env = append(env, shell.PrimeEnvMarkers()...)
	env = append(
		env,
		fmt.Sprintf("PRIME_EVENT=%s", eventName),
		fmt.Sprintf("PRIME_TOOL_NAME=%s", toolName),
		fmt.Sprintf("PRIME_SESSION_ID=%s", sessionID),
		fmt.Sprintf("PRIME_CWD=%s", cwd),
		fmt.Sprintf("PRIME_PROJECT_DIR=%s", projectDir),
	)

	// Extract tool-specific env vars from the JSON input.
	if toolInputJSON != "" {
		if cmd := gjson.Get(toolInputJSON, "command"); cmd.Exists() {
			env = append(env, fmt.Sprintf("PRIME_TOOL_INPUT_COMMAND=%s", cmd.String()))
		}
		if fp := gjson.Get(toolInputJSON, "file_path"); fp.Exists() {
			env = append(env, fmt.Sprintf("PRIME_TOOL_INPUT_FILE_PATH=%s", fp.String()))
		}
	}
...
// Supports both Prime format and Claude Code format (hookSpecificOutput).
