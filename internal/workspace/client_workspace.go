...
import (
	...
	"github.com/charmbracelet/prime/internal/log"
...
func (w *ClientWorkspace) SetConfigFields(scope config.Scope, kv map[string]any) error {
	// The wire layer has no batch endpoint, so each field goes over the
	// existing per-field call. Values are pre-serialized to JSON so that
	// struct types (e.g. []catwalk.Model) survive the socket as raw
	// JSON instead of degrading to []any on the server side.
	for key, value := range kv {
		var raw any = value
		if b, err := json.Marshal(value); err == nil {
			raw = json.RawMessage(b)
		}
		if err := w.client.SetConfigField(context.Background(), w.workspaceID(), scope, key, raw); err != nil {
			return err
		}
		w.refreshWorkspace()
	}
