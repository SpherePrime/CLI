...
func TestRegisterBuiltin_DispatchedThroughRun(t *testing.T) {
	RegisterBuiltin("primetest-echo", func(ctx context.Context, args []string, stdin io.Reader, stdout, stderr io.Writer) error {
		io.WriteString(stdout, "builtin:"+args[1])
		return nil
	})
	t.Cleanup(func() { delete(builtins, "primetest-echo") })

	var stdout bytes.Buffer
	err := Run(t.Context(), RunOptions{
		Command: "primetest-echo hello",
		Cwd:     t.TempDir(),
		Stdout:  &stdout,
	})
	if err != nil {
...
func TestRegisterBuiltin_OverridesPATHBinary(t *testing.T) {
	dir := t.TempDir()
	// Put a fake "primetest-shadowed" binary on PATH that would print
	// "from-path" if it ran.
	binDir := filepath.Join(dir, "bin")
	if err := os.MkdirAll(binDir, 0o755); err != nil {
		t.Fatal(err)
	}
	script := filepath.Join(binDir, "primetest-shadowed")
	if err := os.WriteFile(script, []byte("#!/bin/sh\necho from-path\n"), 0o755); err != nil {
		t.Fatal(err)
	}
	t.Setenv("PATH", binDir+string(os.PathListSeparator)+os.Getenv("PATH"))

	RegisterBuiltin("primetest-shadowed", func(ctx context.Context, args []string, stdin io.Reader, stdout, stderr io.Writer) error {
		io.WriteString(stdout, "from-builtin")
		return nil
	})
	t.Cleanup(func() { delete(builtins, "primetest-shadowed") })

	var stdout bytes.Buffer
	err := Run(t.Context(), RunOptions{
		Command: "primetest-shadowed",
		Cwd:     dir,
		Env:     os.Environ(),
		Stdout:  &stdout,
	})
