...
type RunOptions struct {
	...
	// the Prime process cwd — hooks and the bash tool have different
...
// Prime handler stack. Shared by the stateless [Run] entrypoint and the
...
// standard Prime middleware chain (builtins, script dispatch, block list)
...
// when sourcing framework files can send SIGINT/SIGTERM to Prime's process
...
// regardless of the caller's environment because Prime shells are never
...
//  1. builtins first (so Prime's in-process jq wins over any PATH binary);
...
// builtinHandler returns middleware that dispatches recognized Prime
