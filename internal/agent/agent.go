// Package agent is the core orchestration layer for Prime AI agents.
//
...
type SessionAgentCall struct {
	...
	// reliable completion contract (e.g. `prime run` against a
	...
	// event, so non-interactive clients (e.g. `prime run`) don't
...
// RunID, e.g. `prime run`, would otherwise hang). Uncanceled calls without
...
// that RunID: `prime run` ignores live message events and exits only on a
...
// so callers waiting on those RunIDs (e.g. `prime run`) are not left
...
// caller waiting on RunComplete (e.g. `prime run` with a RunID) always
...
func (a *sessionAgent) Run(ctx context.Context, call SessionAgentCall) (result *fantasy.AgentResult, retErr error) {
	...
	if call.Accepted != nil && a.canceledBySeq(call.SessionID, call.Accepted.seq) {
		...
		// `prime run`, which ignores message events and blocks on
	...
	if err != nil {
		...
		if isCancelErr {
			...
			currentAssistant.AddFinish(message.FinishReasonError, "Unauthorized", `Please re-authenticate with Hyper. You can also run "prime auth" to re-authenticate.`)
...
func (a *sessionAgent) getCacheControlOptions() fantasy.ProviderOptions {
	if t, _ := strconv.ParseBool(os.Getenv("PRIME_DISABLE_ANTHROPIC_CACHE")); t {
