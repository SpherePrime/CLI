...
import (
	...
	openaioauth "github.com/dwertyfa288/CLI/internal/oauth/openai"
	...
	"github.com/dwertyfa288/CLI/vendordeps/fantasy/providers/azure"
	...
	openaisdk "github.com/dwertyfa288/CLI/vendordeps/openai/openai-go/v3/option"
...
func (c *coordinator) run(ctx context.Context, accept *AcceptedRun, sessionID string, prompt string, attachments ...message.Attachment) (*fantasy.AgentResult, error) {
	...
	if err := c.refreshTokenIfExpired(ctx, providerCfg); err != nil {
		// NOTE(@dwertyfa288): We don't return here because the event handling to ask the user to reauthenticate
	...
	// retry's success, and `prime run` would exit on the stale error
...
func (c *coordinator) buildTools(ctx context.Context, agent config.Agent, isSubAgent bool) ([]fantasy.AgentTool, error) {
	...
	logFile := filepath.Join(c.cfg.Config().Options.DataDirectory, "logs", "prime.log")
...
func (c *coordinator) buildProvider(providerCfg config.ProviderConfig, model config.SelectedModel, isSubAgent bool) (fantasy.Provider, error) {
	...
	case openaicompat.Name, hyper.Name:
		...
		case hyper.Name:
			...
			headers["x-prime-id"] = event.GetID()
