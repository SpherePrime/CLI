...
// event into a closure nobody reads, and subscribers (`prime run`)
...
// publish a RunComplete for its RunID, hanging a `prime run` caller that
...
// than internal bookkeeping, which is the contract a `prime run` caller
...
// bookkeeping slice, not the published event a `prime run` caller awaits.
