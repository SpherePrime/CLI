...
// clients within the Prime application.
package mcp
...
func updateState(name string, state State, err error, client *ClientSession, counts Counts, opts ...stateOpt) {
	...
	case StateError:
		...
		// longer call (without that, prime_info / the `/mcp` menu and the
...
// transportWrapper is implemented by every transport decorator prime layers
