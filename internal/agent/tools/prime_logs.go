...
const PrimeLogsToolName = "prime_logs"

//go:embed prime_logs.md.tpl
var primeLogsDescriptionTmpl []byte

var primeLogsDescriptionTpl = template.Must(
	template.New("primeLogsDescription").
		Parse(string(primeLogsDescriptionTmpl)),
)

type primeLogsDescriptionData struct {
	DefaultLines int
	MaxLines     int
}

func primeLogsDescription() string {
	return renderTemplate(primeLogsDescriptionTpl, primeLogsDescriptionData{
...
type PrimeLogsParams struct {
	Lines int `json:"lines,omitempty" description:"Number of recent log entries to return (default 50, max 100)"`
}

func NewPrimeLogsTool(logFile string) fantasy.AgentTool {
	return fantasy.NewAgentTool(
		PrimeLogsToolName,
		primeLogsDescription(),
		func(ctx context.Context, params PrimeLogsParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			result := runPrimeLogs(logFile, params)
			return fantasy.NewTextResponse(result), nil
		},
	)
}

// runPrimeLogs reads and formats the last N log entries from the given file.
func runPrimeLogs(logFile string, params PrimeLogsParams) string {
