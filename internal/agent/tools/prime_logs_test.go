...
func createTestLogFile(t *testing.T, entries []map[string]any) string {
	t.Helper()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "prime.log")
...
func TestNewPrimeLogsTool(t *testing.T) {
	t.Parallel()
	tool := NewPrimeLogsTool("/tmp/test.log")
	require.NotNil(t, tool)
	require.Equal(t, PrimeLogsToolName, tool.Info().Name)
}

func TestPrimeLogs_HappyPath(t *testing.T) {
	t.Parallel()
	...
	result := runPrimeLogs(logFile, PrimeLogsParams{Lines: 3})
...
func TestPrimeLogs_DefaultLines(t *testing.T) {
	t.Parallel()
	...
	result := runPrimeLogs(logFile, PrimeLogsParams{Lines: 0})
...
func TestPrimeLogs_MaxCap(t *testing.T) {
	t.Parallel()
	...
	result := runPrimeLogs(logFile, PrimeLogsParams{Lines: 200})

	lines := strings.Split(result, "\n")
	require.Len(t, lines, 100)
}

func TestPrimeLogs_MissingFile(t *testing.T) {
	t.Parallel()
	result := runPrimeLogs("/nonexistent/path/prime.log", PrimeLogsParams{Lines: 50})
	require.Contains(t, result, "No log file found")
}

func TestPrimeLogs_EmptyFile(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "prime.log")
	...
	result := runPrimeLogs(logFile, PrimeLogsParams{Lines: 50})
	require.Contains(t, result, "Log file is empty")
}

func TestPrimeLogs_MalformedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "prime.log")
	...
	result := runPrimeLogs(logFile, PrimeLogsParams{Lines: 10})
...
func TestPrimeLogs_ExtraFieldsSorted(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runPrimeLogs(logFile, PrimeLogsParams{Lines: 1})
...
func TestPrimeLogs_NonStringValues(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestPrimeLogs_Redaction(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestPrimeLogs_ReservedFields(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
		{
			"time":   time.Now().Format(time.RFC3339),
...
func TestPrimeLogs_OversizedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "prime.log")
...
func TestPrimeLogs_PartialTrailingLine(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "prime.log")
...
func TestPrimeLogs_ValueQuoting(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestPrimeLogs_ChronologicalOrder(t *testing.T) {
	t.Parallel()
...
func TestPrimeLogs_TimeOnlyFormat(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestPrimeLogs_LevelVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runPrimeLogs(logFile, PrimeLogsParams{Lines: 5})
...
func TestPrimeLogs_SourceVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
