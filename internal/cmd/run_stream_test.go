...
// with reason=tool_use and used to terminate `prime run` early via
...
// subscribers (e.g. a TUI watching workspace events while `prime
...
// non-nil error from `prime run` so shells and CI catch it via
// exit status.
...
// run (e.g. Ctrl+C while `prime run` waits) exits cleanly rather
...
// queue scenario: `prime run --continue` attaches to a session
