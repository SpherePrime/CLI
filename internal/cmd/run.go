...
import (
	...
	"github.com/charmbracelet/prime/internal/format"
	"github.com/charmbracelet/prime/internal/herdr"
...
curl https://charm.land | prime run "Summarize this website"

# Read from a file
prime run "What is this code doing?" <<< prrr.go

# Redirect output to a file
prime run "Generate a hot README for this project" > MY_HOT_README.md

# Run in quiet mode (hide the spinner)
prime run --quiet "Generate a README for this project"

# Run in verbose mode (show logs)
prime run --verbose "Generate a README for this project"

# Use a specific reasoning effort
# Levels depend on the model, unsupported values are rejected
# with the accepted values listed
prime run --reasoning-effort high "What is the meaning of life?"

# Continue a previous session
prime run --session {session-id} "Follow up on your last response"

# Continue the most recent session
prime run --continue "Follow up on your last response"
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if useClientServer() {
			...
			if !ws.Config.IsConfigured() {
				return fmt.Errorf("no providers configured - please run 'prime' to set up a provider interactively")
			}
...
// streaming SSE events into a final, complete stdout for `prime run`.
...
func (s *runStream) handle(ev any, stopSpinner func()) (done bool, err error) {
	...
	case pubsub.Event[proto.RunComplete]:
		...
		// source of the regression where `prime run` exited
...
// prefix returned by prime session list.
