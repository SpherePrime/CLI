...
func init() {
	...
	rootCmd.PersistentFlags().StringP("data-dir", "D", "", "Custom prime data directory")
...
cat README.md | prime run "make this more glamorous" > GLAMOROUS_README.md

# Run with debug logging in a specific directory
prime --debug --cwd /path/to/project
...
// session title and the hint to resume it with `prime -s <id>`. The banner
...
// enabled via the PRIME_CLIENT_SERVER environment variable.
func useClientServer() bool {
	v, _ := strconv.ParseBool(os.Getenv("PRIME_CLIENT_SERVER"))
	return v
}
...
// PRIME_CLIENT_SERVER=1, it connects to a server process and returns a
...
func setupLocalWorkspace(cmd *cobra.Command) (workspace.Workspace, func(), error) {
	...
	logFile := filepath.Join(cfg.Options.DataDirectory, "logs", "prime.log")
	primelog.Setup(logFile, debug)
...
func connectToServer(cmd *cobra.Command) (*client.Client, *proto.Workspace, func(), error) {
	...
	if ws.Config != nil {
		logFile := filepath.Join(ws.Config.Options.DataDirectory, "logs", "prime.log")
		primelog.Setup(logFile, debug)
	}
...
func replaceExitingServer(cmd *cobra.Command, hostURL *url.URL) error {
	...
	if err := spawnAndWaitReady(cmd, hostURL); err != nil {
		return fmt.Errorf("failed to initialize prime server: %v", err)
	}
	return nil
}
...
func ensureServer(cmd *cobra.Command, hostURL *url.URL) error {
	...
	// server log file. primelog.Setup uses sync.Once internally, so the
	// later call from connectToServer becomes a no-op.
	debug, _ := cmd.Flags().GetBool("debug")
	logFile := filepath.Join(config.GlobalCacheDir(), "server-"+safeHostName(hostURL), "prime.log")
	primelog.Setup(logFile, debug)
...
// $XDG_CACHE_HOME/prime/server-<safeHost>/start.lock.
//
...
// Overridable via PRIME_SERVER_READY_TIMEOUT (parsed as a Go duration).
func serverReadyTimeout() time.Duration {
	const def = 10 * time.Second
	v := os.Getenv("PRIME_SERVER_READY_TIMEOUT")
	if v == "" {
		return def
	}
...
func startDetachedServer(cmd *cobra.Command, hostURL *url.URL) error {
	...
	if err := c.Start(); err != nil {
		return fmt.Errorf("failed to start prime server: %v", err)
	}

	if err := c.Process.Release(); err != nil {
		return fmt.Errorf("failed to detach prime server process: %v", err)
	}
...
func shouldEnableMetrics(cfg *config.Config) bool {
	if v, _ := strconv.ParseBool(os.Getenv("PRIME_DISABLE_METRICS")); v {
		return false
	}
...
func createDotPrimeDir(dir string) error {
