...
// multiplexer. When Prime runs inside a herdr-managed pane it reports
...
// Client reports Prime agent state to a running herdr instance.
type Client struct {
	socketPath string
	paneID     string
...
// call from environment variables. Returns nil when Prime is not
...
// is not strictly greater. Because prime seeds seq from the wall
// clock at startup (see newFromEnv), a restarted prime in the same
...
func (c *Client) newRequestLocked(method, idPrefix, state string) reportRequest {
	...
	return reportRequest{
		ID:     fmt.Sprintf("prime:%s:%d", idPrefix, time.Now().UnixNano()),
		Method: method,
		Params: reportParams{
			PaneID:         c.paneID,
			Source:         "prime",
