...
// UpdateAvailable is published when a newer Prime release is detected
...
// completion contract (notably `prime run` in client/server mode)
...
// `prime run`, which may attach to a busy session whose currently
