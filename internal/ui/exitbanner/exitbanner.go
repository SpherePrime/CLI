// Package exitbanner renders what Prime prints after the TUI exits.
package exitbanner
...
func logoSection(contentWidth int) string {
	...
	primeLogo := logo.Render(t.Logo.GradCanvas, version.Version, true, logo.Opts{
	...
	return primeLogo + "\n" +
		lipgloss.NewStyle().Width(contentWidth).Render("Thanks for using Prime! "+randomExitMessage())
}

// sessionResumeLines returns the "Session  <title>\nContinue prime -s <hash>"
