...
import (
	...
	agenttools "github.com/dwertyfa288/CLI/internal/agent/tools"
	...
	"github.com/dwertyfa288/CLI/internal/ui/completions"
	...
	"github.com/dwertyfa288/CLI/vendordeps/dwertyfa288/x/editor"
...
type UI struct {
	...
	// copy/paste, right-click, and scrolling instead of Prime.
	mouseEnabled bool
...
func New(com *common.Common, initialSessionID string, continueLast bool) *UI {
	...
	// Copying is handled by prime's keymap (Editor.CopySelection) so it can
	// use prime's clipboard backend and user feedback; disable the
...
func (m *UI) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case app.UpdateAvailableMsg:
		text := fmt.Sprintf("Prime update available: v%s → v%s.", msg.CurrentVersion, msg.LatestVersion)
		if msg.IsDevelopment {
			text = fmt.Sprintf("This is a development version of Prime. The latest version is v%s.", msg.LatestVersion)
		}
		ttl := 10 * time.Second
...
func (m *UI) handleConnectionEvent(msg workspace.ConnectionEvent) []tea.Cmd {
	info := util.InfoMsg{
		...
		Msg:  "Lost connection to the Prime server — reconnecting…",
		TTL:  30 * time.Second,
	}
	switch msg.State {
...
func (m *UI) Draw(scr uv.Screen, area uv.Rectangle) *tea.Cursor {
	...
	if os.Getenv("PRIME_UI_DEBUG") == "true" {
...
const (
	planModeBannerMsg = "Plan with Prime before generating any code."
