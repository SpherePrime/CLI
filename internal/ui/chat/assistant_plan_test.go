...
func TestAssistantMessageItem_PlanStreamingCardIsOpenUntilMarker(t *testing.T) {
	...
	streaming := NewAssistantMessageItem(&sty, mk("<!-- PRIME_PLAN_START -->\n\n# Plan\n\nIn progress", false)).(*AssistantMessageItem)
	...
	require.NotContains(t, got, "PRIME_PLAN_START", "the start marker must be stripped from the card")

	done := NewAssistantMessageItem(&sty, mk("<!-- PRIME_PLAN_START -->\n\n# Plan\n\nDone\n\n<!-- PRIME_PLAN_READY -->", true)).(*AssistantMessageItem)
	...
	require.NotContains(t, closed, "PRIME_PLAN_START", "the start marker must be stripped from the card")
	...
	interrupted := NewAssistantMessageItem(&sty, mk("<!-- PRIME_PLAN_START -->\n\nHalf a plan", true)).(*AssistantMessageItem)
	...
	require.NotContains(t, cut, "PRIME_PLAN_START", "the start marker must not leak into rendered output")
}
...
func TestAssistantMessageItem_NonPlanRepliesHaveNoPlanCard(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "marker mentioned in prose", text: "The marker <!-- PRIME_PLAN_READY --> is not on its own line."},
	}
