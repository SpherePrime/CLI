...
// session's messages. Point PRIME_BENCH_SESSION at a full session id and
// PRIME_BENCH_DATADIR at the prime data dir (defaults to ./.prime).
//
//	PRIME_BENCH_SESSION=e6368d820207a406 go test ./internal/ui/chat/ \
//	  -run x -bench BenchmarkResizeSession -benchtime 20x -cpuprofile /tmp/cpu.out
func BenchmarkResizeSession(b *testing.B) {
	sessionID := os.Getenv("PRIME_BENCH_SESSION")
	if sessionID == "" {
		b.Skip("set PRIME_BENCH_SESSION to a full session id")
	}
	dataDir := os.Getenv("PRIME_BENCH_DATADIR")
	if dataDir == "" {
		dataDir = ".prime"
	}
